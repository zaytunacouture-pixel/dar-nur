#!/usr/bin/env node
// scripts/import-supplier.mjs — prototype d'import fournisseur → Dar Nūr.
//
// LE FOURNISSEUR N'EST JAMAIS NOMMÉ DANS LE DÉPÔT (dépôt public, GitHub Pages
// sert tout fichier committé). Tout ce qui l'identifie — code, URL de son API,
// correspondance de ses catégories, noms à bannir des textes, motif de ses
// fichiers image — vit dans `supplier.config.local.json` à la racine du dépôt,
// ignoré par Git (voir SUPPLIER_CONFIG_EXAMPLE ci-dessous). Le fournisseur
// n'est identifiable que dans les données privées Supabase (product_sources).
//
// Périmètre volontairement limité : une liste explicite d'identifiants
// fournisseur (`--ids`), jamais le catalogue entier. Sans `--apply`, le script
// est un DRY-RUN complet : il lit le fournisseur et Supabase, calcule le plan
// (créations, rapprochements, champs modifiés, prix avant/après, variantes,
// marques, images optimisées, lignes product_sources) et l'écrit dans un
// dossier de travail — sans écrire un seul octet dans Supabase ni dans assets/.
//
// Usage :
//   node scripts/import-supplier.mjs --ids 123,456 [--work <dossier>] [--exclude-images <url,url>]
//   node scripts/import-supplier.mjs --ids ... --apply [--auth cli]   # après validation du dry-run
//   node scripts/import-supplier.mjs --verify <dossier-d-application>   # contrôle post-import
//   node scripts/import-supplier.mjs --rollback <dossier-de-sauvegarde>
//
// Écriture (`--apply` / `--rollback` / `--verify`) : `--auth cli` (clé secrète du
// projet obtenue en mémoire via la CLI Supabase authentifiée par `supabase
// login`) ou, à défaut, session administrateur par e-mail + mot de passe lus
// dans DN_ADMIN_EMAIL / DN_ADMIN_PASSWORD (jamais écrits dans le dépôt).
// L'écriture est refusée sans la table `product_sources`
// (supabase/sql/product_sources.sql) : la provenance privée fait partie du
// contrat, un produit importé sans sa source n'existe pas.
//
// Source : Store API WooCommerce publique du fournisseur (`/wp-json/wc/store/v1/
// products`), complétée par le JSON-LD Product de la page (seule source du
// GTIN/EAN). Requêtes espacées (DELAY_MS), un seul passage par produit, aucune
...
//   - Prix : prix public fournisseur effectif (promo incluse) arrondi à l'euro le
//     plus proche. Nouveau produit = ce prix. Produit existant = aligné seulement
//     si le prix fournisseur arrondi est INFÉRIEUR au prix Dar Nūr, jamais augmenté.
...
//     contenant l'un des noms bannis (config locale) est refusé, les
//     formulations « senteur / inspiration <marque tierce> » sont exclues.
//   - Aucune donnée inventée : un champ absent chez le fournisseur reste vide.
//   - purchase_price reste NULL (« prix d'achat à renseigner »).
//
// Zéro dépendance npm (fetch natif, Node ≥ 18). L'optimisation d'image et la
// détection du filigrane sont déléguées à scripts/import-supplier-images.py et
// scripts/import-supplier-watermark.py (Pillow + numpy).
...
const OPTIMIZER = path.join(ROOT, 'scripts', 'import-supplier-images.py');
const WATERMARK_DETECTOR = path.join(ROOT, 'scripts', 'import-supplier-watermark.py');
const SUPPLIER_CONFIG_PATH = path.join(ROOT, 'supplier.config.local.json');
// Gabarits du filigrane fournisseur : cartes de contours .npz rangées HORS du
// dépôt (dossier local ignoré par Git), chemins fournis par la config locale.
// Mesuré sur 34 images de l'échantillon : filigranées ≥ 0,516, propres ≤ 0,397.
// Le contrôle visuel de la planche-contact reste obligatoire : premier filtre.
...
// Configuration locale du fournisseur (jamais committée) — exemple :
const SUPPLIER_CONFIG_EXAMPLE = {
  code: 'fournisseur-a',                 // valeur de product_sources.supplier
  baseUrl: 'https://exemple.invalid',    // site WooCommerce du fournisseur
  storeApiPath: '/wp-json/wc/store/v1',
  forbiddenNames: ['Nom Du Fournisseur'],// mots entiers refusés dans tout champ public
  imageNameExcludePattern: 'logo',       // regex (i) sur le nom de fichier des images à ignorer
  categoryMap: { 'parfum-x': 'parfums' },// slug catégorie fournisseur → catégorie Dar Nūr
  categoryIgnore: ['promo'],             // catégories fournisseur purement commerciales
  watermarkTemplates: ['supplier.local/watermark-template-1.npz'], // gabarits .npz, locaux, jamais committés
};
const SUPPLIER = loadSupplierConfig();
const WATERMARK_TEMPLATES = SUPPLIER.watermarkTemplates.map(p => path.resolve(ROOT, p));
const STORE_API = `${SUPPLIER.baseUrl.replace(/\/$/, '')}${SUPPLIER.storeApiPath}`;
...
// Catégorie fournisseur (slug) → catégorie Dar Nūr, depuis la config locale.
// Toute catégorie absente de la table fait abandonner le produit (garde-fou :
// rien n'est classé « par défaut »).
const CATEGORY_MAP = SUPPLIER.categoryMap;
// Catégories fournisseur purement commerciales (jamais une catégorie produit).
const CATEGORY_IGNORE = new Set(SUPPLIER.categoryIgnore);

// Mots retirés du nom pour obtenir le « cœur » comparable entre fournisseur et Dar Nūr
...
function loadSupplierConfig() {
  if (!existsSync(SUPPLIER_CONFIG_PATH)) {
    fail(`Configuration fournisseur absente : ${SUPPLIER_CONFIG_PATH}\n  Créer ce fichier (ignoré par Git) sur le modèle :\n  ${JSON.stringify(SUPPLIER_CONFIG_EXAMPLE, null, 2).replace(/\n/g, '\n  ')}`);
  }
  const cfg = JSON.parse(readFileSync(SUPPLIER_CONFIG_PATH, 'utf8'));
  for (const k of Object.keys(SUPPLIER_CONFIG_EXAMPLE)) {
    if (cfg[k] === undefined) fail(`supplier.config.local.json : clé « ${k} » manquante.`);
  }
  if (!/^[a-z0-9-]+$/.test(cfg.code)) fail('supplier.config.local.json : « code » doit être un identifiant kebab-case.');
  if (!Array.isArray(cfg.forbiddenNames) || !cfg.forbiddenNames.length) fail('supplier.config.local.json : « forbiddenNames » doit lister au moins un nom.');
  if (!Array.isArray(cfg.watermarkTemplates) || !cfg.watermarkTemplates.length) fail('supplier.config.local.json : « watermarkTemplates » doit lister au moins un gabarit .npz local.');
  for (const t of cfg.watermarkTemplates) if (!existsSync(path.resolve(ROOT, t))) fail(`Gabarit de filigrane introuvable : ${t} (à produire localement : python scripts/import-supplier-watermark.py --build-template <capture> <sortie.npz>)`);
  return cfg;
}
...
// ---------------------------------------------------------------------------
// Nettoyage des textes fournisseur
// ---------------------------------------------------------------------------

// Noms du fournisseur, en mots entiers (un nom qui serait le préfixe d'un mot
// courant n'est donc pas concerné) — lus dans la config locale, jamais écrits ici.
const SUPPLIER_NAME_RE = new RegExp(`\\b(?:${SUPPLIER.forbiddenNames.map(n => n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})\\b`, 'i');
...
// Ligne-titre commerciale du fournisseur (« Eau de Parfum pour Femme – X – 100ml – Marque »).
...
function assertNoSupplierMention(obj, where) {
  ...
  if (SUPPLIER_NAME_RE.test(s)) fail(`Garde-fou : nom du fournisseur détecté dans ${where} — import refusé.`);
}

// ---------------------------------------------------------------------------
// Fournisseur (WooCommerce Store API + JSON-LD)
// ---------------------------------------------------------------------------

async function supplierFetch(url, { json = true } = {}) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    throw new Error(`Le fournisseur a répondu ${res.status} ${res.statusText} (${url})`);
  }
  throw new Error(`Fournisseur indisponible après 3 essais (${url})`);
}
...
async function fetchSupplierProduct(id) {
  const p = await supplierFetch(`${STORE_API}/products/${id}`);
  await sleep(DELAY_MS);
  const variations = [];
  for (const v of p.variations || []) {
    variations.push(await supplierFetch(`${STORE_API}/products/${v.id}`));
    await sleep(DELAY_MS);
  }
  let ld = { gtin: null, dateModified: null };
  try {
    ld = extractJsonLd(await supplierFetch(p.permalink, { json: false }));
  } catch (e) {
...
async function loadDarNur(creds) {
  ...
  try {
    sources = await sbGet(creds, `product_sources?select=*&supplier=eq.${SUPPLIER.code}`);
  } catch (e) {
...
// ---------------------------------------------------------------------------
// Extraction fournisseur → structure neutre
// ---------------------------------------------------------------------------
...
function planProduct(x, dn, plannedSlugs, plannedBrands) {
  const item = { supplier: x, warnings: [], decision: null };

  if (!x.category || !CATEGORY_MAP[x.category.slug]) {
    item.decision = 'ABANDON';
    item.warnings.push(`catégorie fournisseur « ${x.category?.name || '?'} » non prise en charge par le prototype`);
    return item;
  }
  ...
  item.match = m;
  const supplierPrice = x.variations.length ? Math.min(...x.variations.map(v => v.price).filter(v => v != null)) : x.price;
  item.supplierPriceRounded = roundEuro(supplierPrice);
  ...
  if (m.product) {
    ...
    } else if (item.supplierPriceRounded != null && p.price_value != null && item.supplierPriceRounded < Number(p.price_value)) {
      patch.price_value = item.supplierPriceRounded;
      priceNote = `baisse ${fmt(p.price_value)} → ${fmt(item.supplierPriceRounded)} (fournisseur ${fmt(supplierPrice)} arrondi)`;
    } else if (item.supplierPriceRounded != null && p.price_value != null) {
      priceNote = `fournisseur ${fmt(supplierPrice)} → arrondi ${fmt(item.supplierPriceRounded)} ≥ Dar Nūr ${fmt(p.price_value)} — prix Dar Nūr conservé`;
    } else {
      priceNote = 'prix Dar Nūr absent ou prix fournisseur inconnu — non touché';
    }
    item.priceNote = priceNote;
    item.patch = patch;
    item.fills = fills;
    item.imagesNote = isEmpty(p.images) ? 'aucune image Dar Nūr — les images fournisseur seraient ajoutées' : `${p.images.length} image(s) Dar Nūr conservée(s), images fournisseur non importées`;
  ...
  const row = {
    ...
    weight: null,
    volume: x.variations.length ? null : (x.contenances[0] || null),
    price_value: item.supplierPriceRounded,
...
async function downloadImages(item, workDir, excludedUrls) {
  ...
  for (const im of item.supplier.images) {
    ...
    if (new RegExp(SUPPLIER.imageNameExcludePattern, 'i').test(path.basename(im.src))) { skipped.push({ src: im.src, reason: 'nom de fichier fournisseur/logo' }); continue; }
...
const runOptimizer = (jobs, sheet) => runPython(OPTIMIZER, { jobs, sheet }, 'import-supplier-images.py');
const runWatermarkDetector = files => runPython(WATERMARK_DETECTOR, { templates: WATERMARK_TEMPLATES, files, threshold: WATERMARK_THRESHOLD }, 'import-supplier-watermark.py').then(r => r.results);
...
function renderReport(plan, mode) {
  const L = [];
  L.push(`# Import fournisseur — ${mode} du ${new Date().toISOString()}`);
  L.push('');
  ...
  for (const it of plan.items) {
    ...
    L.push(`- Fournisseur : ${x.supplier_url} · id ${x.supplier_product_id} · SKU ${x.supplier_sku || '—'} · GTIN ${x.gtin || '—'} · marque ${x.brandNames.join(', ') || '—'} · type ${x.type}`);
    L.push(`- Prix fournisseur : régulier ${fmt(x.regular_price)} · effectif ${fmt(x.price)}${x.on_sale ? ' (promo)' : ''} → arrondi ${fmt(it.supplierPriceRounded)} · dispo ${x.in_stock ? 'oui' : 'NON'}${x.stock_text ? ` (${x.stock_text})` : ''}`);
    ...
    if (it.decision === 'PRODUIT EXISTANT') {
      ...
      if (it.variants.length) {
        ...
        for (const v of it.variants) L.push(`  - ${v.name} · ${fmt(v.price)} (fournisseur ${fmt(v._supplier.regular_price)} → ${fmt(v._supplier.price)}${v._supplier.on_sale ? ' promo' : ''}) · SKU ${v.sku || '—'} · dispo ${v._supplier.in_stock ? 'oui' : 'NON'}${v._supplier.stock_text ? ` (${v._supplier.stock_text})` : ''}`);
      }
    }
    if (it.decision !== 'ABANDON') {
      const s = it.source;
      L.push(`- product_sources (${it.sourceMode}) : supplier=${s.supplier} · id=${s.supplier_product_id} · sku=${s.supplier_sku || '—'} · gtin=${s.gtin || '—'} · régulier ${fmt(s.supplier_regular_price)} · effectif ${fmt(s.supplier_price)} · promo ${s.supplier_on_sale} · dispo ${s.supplier_in_stock} · qté ${s.supplier_stock_qty ?? '—'} · purchase_price = NULL (à renseigner) · modifié chez le fournisseur ${s.supplier_modified_at || '—'}${it.variantSources?.length ? ` · + ${it.variantSources.length} ligne(s) variante` : ''}`);
    }
    if (it.images) {
    ...
    const dropped = [...it.supplier.description.dropped, ...it.supplier.short.dropped];
    if (dropped.length) {
      L.push(`- Textes fournisseur écartés (${dropped.length}) :`);
  ...
  try {
    ...
    for (const it of plan.items) {
      if (it.decision === 'NOUVEAU PRODUIT') {
        ...
        const prev = backup.sources.find(r => r.product_id === it.existing.id && !r.variant_id && r.supplier === SUPPLIER.code);
    ...
    log(`  Rollback possible : node scripts/import-supplier.mjs --rollback "${workDir}"`);
  } catch (e) {
    manifest.error = e.message; await save();
    fail(`Application interrompue : ${e.message}\n  Ce qui a déjà été écrit est listé dans ${manifestPath} — rollback : node scripts/import-supplier.mjs --rollback "${workDir}"`);
  }
}
...
// modifiée (instantané pris juste avant l'écriture), aucun nom du fournisseur
// dans les champs publics (noms bannis de la config locale), table product_sources illisible en anonyme.
...
async function verifyApply(workDir, creds, anonCreds) {
  ...
  const sources = await sbGet(creds, `product_sources?select=*&supplier=eq.${SUPPLIER.code}`);
  ...
  for (const it of plan.items.filter(i => i.decision === 'NOUVEAU PRODUIT')) {
    ...
    check(!SUPPLIER_NAME_RE.test(JSON.stringify({ n: p.name, t: p.tagline, d: p.description, c: p.composition, a: p.accordions, b: p.benefits, i: p.images })), `${it.slug} : aucun nom du fournisseur dans les champs publics`);
  }
  L.push('', '## Marques');
  for (const b of plan.brandsToCreate) {
  ...
  for (const it of plan.items.filter(i => i.decision !== 'ABANDON')) {
    ...
    check(!!prod && prod.supplier_product_id === it.supplier.supplier_product_id, `${slug} : ligne produit (id fournisseur ${prod?.supplier_product_id})`);
    if (prod) {
      check(prod.purchase_price === null, `${slug} : purchase_price = ${prod.purchase_price} (à renseigner)`);
      check(Number(prod.supplier_price) === Number(it.supplier.price) && Number(prod.supplier_regular_price) === Number(it.supplier.regular_price), `${slug} : prix fournisseur ${fmt(prod.supplier_price)} (régulier ${fmt(prod.supplier_regular_price)})`);
      check(prod.supplier_in_stock === it.supplier.in_stock && (prod.supplier_stock_qty ?? null) === (it.supplier.stock_qty ?? null), `${slug} : dispo ${prod.supplier_in_stock}, qté ${prod.supplier_stock_qty ?? '—'}`);
      check((prod.gtin ?? null) === (it.supplier.gtin ?? null), `${slug} : gtin ${prod.gtin ?? '—'}`);
    }
...
async function main() {
  ...
  if (!args.ids.length) fail('Aucun identifiant : --ids 123,456,…  (jamais le catalogue entier)');
  if (args.ids.length > 20) fail(`${args.ids.length} identifiants : ce prototype refuse plus de 20 produits par exécution.`);

  const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
  const workDir = path.resolve(args.work || path.join(ROOT, 'backups', 'supplier-import', `${args.apply ? 'apply' : 'dry-run'}-${stamp}`));
  ...
  for (const id of args.ids) {
    log(`Fournisseur ${id}…`);
    const fetched = await fetchSupplierProduct(id);
  ...
  // Garde-fous avant écriture : jamais le nom du fournisseur dans un champ écrit, nouveaux produits inactifs.
  for (const it of plan.items) {
